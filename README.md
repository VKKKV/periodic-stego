...
- **Extraction tools** exposes structured PNG/GIF trailing-data extraction with raw, ASCII-bit or strict Base64 conversion, and uncompressed indexed-BMP bit extraction between zero-pair sentinels. Choose bit order, symbols and optional bit insertion explicitly; no file-specific payload lengths or missing-bit repair is assumed. Input is capped at 32 MiB and output at 1 MiB. Parameter/source changes invalidate prior output.
- **Raw PNG bitplane / bitstream** uses native non-interlaced 8-bit PNG decoding, preserving hidden RGB under alpha zero. Choose a channel/bit for preview, or ordered channels, row/column traversal, byte packing, bit offset and output byte count for extraction. Other PNG depths and Adam7 fail explicitly without Canvas fallback. Previews are bounded; downloads preserve all extracted bytes. There is no challenge catalog, answer preset or automatic payload inference.
