...
- **Forensic extraction extension — current**: 460 Web unit tests pass across 17 files, plus a Chromium E2E smoke test for the generic tool operations; channel differences, median anomalies, prime masks, coordinate/Morse conversion, byte inversion/XOR, coordinate plotting and structured JPEG inspection are covered by reference cases. The production build and Prettier pass. The tool UI exposes all operations as explicit methods; BarcodeDetector is capability-gated and reports an unavailable-browser error rather than inferring or substituting a decoder. Raw-pixel operations run in bounded, cancellable UI jobs; measured implementation does not currently warrant a second Worker because native PNG decompression already yields between chunks and the new masks are bounded by the 16-megapixel input cap.
